...
class FFmpegRecorder:
    ...
    def start(self):
        ...
        self.audio_path = os.path.abspath(os.path.join(self.output_dir, f'audio_{timestamp}.aac'))
    ...
    def mux(self, video_path, audio_path, output_path=None):
        if not output_path:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            output_path = os.path.abspath(os.path.join(self.output_dir, f'muxed_{timestamp}.mp4'))
        ...
        '-i', video_path,
        '-i', audio_path,
        '-c:v', 'copy',   # não recodifica vídeo
        '-c:a', 'copy',   # não recodifica áudio AAC
        '-shortest',      # termina junto
        output_path
        ]
